...
const LivePlayers = () => (
  <div style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
    <div style={{ display: "flex", alignItems: "center" }}>
      {[ASSETS.avatarFox, ASSETS.avatarBear, ASSETS.avatarFrog, ASSETS.avatarPanda].map((src, i) => (
        <PixelImg key={i} src={src} size={34} alt="" style={{ marginLeft: i ? -12 : 0, borderRadius: 99, border: "2px solid var(--frame)" }} />
      ))}
    </div>
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 800, color: "var(--ink-soft)" }}>
      <span style={{ width: 7, height: 7, borderRadius: 99, background: "var(--live-red)", boxShadow: "0 0 0 3px rgba(252,25,25,.2)", animation: "waffles-v2-pulse 1.5s infinite" }} />
      2,000+ playing now
    </span>
  </div>
);
...
const PrizeVisual = () => (
  <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
    <PixelImg src={ASSETS.chestRainbow} size={160} alt="" style={{ filter: "drop-shadow(0 14px 26px rgba(0,0,0,.45))" }} />
    <PixelImg src={ASSETS.trophy} size={64} alt="" style={{ position: "absolute", top: -12, right: 2, transform: "rotate(8deg)", filter: "drop-shadow(0 6px 12px rgba(0,0,0,.4))" }} />
  </div>
);
...
const PITCH: Slide = {
  ...
  body: (
    <>
      ...
      players <strong style={{ color: "var(--ink)" }}>split the prize pool in real USDT</strong>.
    </>
  ),
  visual: <PrizeVisual />,
};
...
const DemoSlide = ({ question, onResolve }: { question: DemoQuestion; onResolve: (wasCorrect: boolean) => void }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, width: "100%" }}>
      <div style={{ fontSize: 12, fontWeight: 800, color: "var(--leaf)", letterSpacing: 2, textTransform: "uppercase" }}>
        TRY IT — GUESS THE ANSWER
      </div>
      <h2 style={{ fontFamily: "var(--font-hero)", fontWeight: 800, fontSize: 24, lineHeight: 1.12, color: "var(--ink)", textAlign: "center", margin: 0, maxWidth: 320 }}>
        {question.content}
      </h2>

      {question.mediaUrl && (
        <div style={{ width: "100%", maxWidth: 280, borderRadius: 12, overflow: "hidden", border: "2px solid var(--frame)", background: "#000" }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={question.mediaUrl} alt="" loading="lazy" style={{ width: "100%", display: "block", aspectRatio: "16 / 9", objectFit: "cover" }} />
        </div>
      )}
      ...
      {answered && (
        <div style={{ fontFamily: "var(--font-display)", fontSize: 16, color: isCorrect ? "var(--leaf)" : "var(--ink-mute)", animation: "waffles-v2-onb-in .18s var(--ease-out-quart)" }}>
...
export const OnboardingScreen = ({
  ...
  const flow = useMemo<("pitch" | "demo" | "account")[]>(
    () => (demo && demo !== "loading" ? ["pitch", "demo", "account"] : ["pitch", "account"]),
    [demo],
  );
  ...
  useEffect(() => {
    if (!startedRef.current) {
      ...
      trackClientEvent(AnalyticsEvent.OnboardingStarted, { step_index: idx, step_id: stepId, total_steps: totalSteps });
    }
    trackClientEvent(AnalyticsEvent.OnboardingSlideViewed, { step_index: idx, step_id: stepId, total_steps: totalSteps });
  ...
  useEffect(() => {
    ...
    trackClientEvent(AnalyticsEvent.OnboardingUsernameEntered, { step_index: idx, step_id: stepId, username_length: username.trim().length });
  ...
  const createAccount = () => {
    ...
    setSigninError(null);
    setConnecting(true);
    trackClientEvent(AnalyticsEvent.OnboardingSignupClicked, { step_index: idx, step_id: stepId });
    void signIn()
      .then((ok) => {
        setConnecting(false);
        if (!ok) {
          trackClientEvent(AnalyticsEvent.OnboardingFailed, { step_index: idx, step_id: stepId, reason: "signin_failed" });
          setSigninError("Couldn’t create your account — check your wallet and try again.");
          return;
        }
        proto.setUsername(name);
        trackClientEvent(AnalyticsEvent.OnboardingCompleted, { step_index: idx, step_id: stepId, username_length: name.length, demo_correct: demoWasCorrect });
        trackClientEvent(AnalyticsEvent.UserOnboarded, { step_index: idx, step_id: stepId, username_length: name.length });
        setWelcomeOpen(true);
      })
      .catch(() => {
        setConnecting(false);
  ...
  const openLegal = (tab: LegalTab) => {
    trackClientEvent(AnalyticsEvent.OnboardingLegalOpened, { step_index: idx, step_id: stepId, legal_tab: tab });
    setLegalTab(tab);
  };
  ...
  const accountKicker = demoWasCorrect ? "YOU'RE A NATURAL" : "READY WHEN YOU ARE";
  ...
  return (
    <Phone>
      ...
      >
        {accountStep ? (
          <>
            <div style={{ minHeight: 180, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18 }}>
              <PixelImg src={ASSETS.wally} size={132} alt="" style={{ animation: "waffles-v2-wally-idle 4s ease-in-out infinite" }} />
            </div>
            <div style={{ fontSize: 12, fontWeight: 800, color: "var(--maple-500)", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>
              {accountKicker}
            </div>
            <div style={{ fontFamily: "var(--font-hero)", fontWeight: 800, fontSize: 36, lineHeight: 1.05, color: "var(--ink)", marginBottom: 12 }}>
              {accountTitle}
            </div>
            <div style={{ fontSize: 15, lineHeight: 1.5, fontWeight: 600, color: "var(--ink-mute)", maxWidth: 320, marginBottom: 18 }}>
              Pick a username and we&apos;ll connect your wallet, so your winnings stay yours.{" "}
              <strong style={{ color: "var(--leaf)" }}>Your first entry is half price.</strong>
            </div>
          ...
          <>
            <div style={{ minHeight: 200, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 26 }}>
              {PITCH.visual}
            </div>
            <div style={{ fontSize: 12, fontWeight: 800, color: PITCH.kickerColor, letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>
              {PITCH.kicker}
            </div>
            <div style={{ fontFamily: "var(--font-hero)", fontWeight: 800, fontSize: 38, lineHeight: 1.05, color: "var(--ink)", marginBottom: 14 }}>
              {PITCH.title}
            </div>
            <div style={{ fontSize: 16, lineHeight: 1.5, fontWeight: 600, color: "var(--ink-mute)", maxWidth: 320, marginBottom: 18 }}>
              {PITCH.body}
            </div>
          ...
          {Array.from({ length: totalSteps }).map((_, i) => (
            <div
              ...
              style={{
                ...
                height: 8,
                borderRadius: 99,
                background: i === idx ? "var(--maple-500)" : "rgba(255,255,255,.18)",
        ...
        {!demoStep && (
          <div className="cta-row">
            <button
              ...
              style={!canContinue || pitchWaiting ? { opacity: 0.5, cursor: "not-allowed" } : undefined}
            >
              {ctaLabel}
            </button>
          </div>
        ...
        >
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "0 28px", position: "relative", zIndex: 1 }}>
            <div style={{ position: "relative", marginBottom: 14 }}>
              <PixelImg src={ASSETS.chestRainbow} size={140} alt="" style={{ filter: "drop-shadow(0 0 28px rgba(255,210,77,.5))" }} />
              <PixelImg src={ASSETS.trophy} size={58} alt="" style={{ position: "absolute", top: -10, right: -6, transform: "rotate(8deg)" }} />
            </div>
